...
def build_multi_lists(labeled_candidates: pd.DataFrame, *, top_n: int = 30, as_of_date: str | None = None) -> dict[str, object]:
    ...
    if frame.empty:
        lists = [_list_payload(list_id, as_of_date=as_of_date, top_n=top_n, items=[]) for list_id in LIST_IDS]
    ...
    lists = [
        _list_payload(
            ...
            top_n=top_n,
            items=_items(_high_confidence(frame), ["total_score", "confidence", "risk_score"], [False, False, False], top_n),
        ),
        _list_payload(
            "trend_leaders",
            as_of_date=resolved_date,
            top_n=top_n,
            items=_items(_with_tag(frame, PRIMARY_TREND_LEADER, exclude_high_risk=True), ["trend_score", "momentum_score", "relative_strength_score"], [False, False, False], top_n),
        ),
        _list_payload(
            "long_term_stable",
            as_of_date=resolved_date,
            top_n=top_n,
            items=_items(_with_tag(frame, PRIMARY_LONG_TERM_STABLE, exclude_high_risk=True), ["risk_score", "liquidity_score", "trend_score"], [False, False, False], top_n),
        ),
        _list_payload(
            "breakout_watch",
            as_of_date=resolved_date,
            top_n=top_n,
            items=_items(_with_tag(frame, PRIMARY_BREAKOUT), ["momentum_score", "trend_score", "relative_strength_score"], [False, False, False], top_n, require_risk_note=True),
        ),
        _list_payload(
            "accumulation_watch",
            as_of_date=resolved_date,
            top_n=top_n,
            items=_items(_with_tag(frame, PRIMARY_ACCUMULATION, exclude_high_risk=True), ["total_score", "risk_score", "trend_score"], [False, False, False], top_n),
        ),
        _list_payload(
            "rebound_watch",
            as_of_date=resolved_date,
            top_n=top_n,
            items=_items(_with_tag(frame, PRIMARY_REBOUND), ["momentum_score", "risk_score", "total_score"], [False, False, False], top_n, require_risk_note=True),
        ),
        _list_payload(
            "high_risk_active",
            as_of_date=resolved_date,
            top_n=top_n,
            items=_items(_high_risk(frame), ["total_score", "momentum_score", "liquidity_score"], [False, False, False], top_n, require_risk_note=True),
        ),
        _list_payload(
            "insufficient_data",
            as_of_date=resolved_date,
            top_n=top_n,
            items=_items(frame[frame["primary_type"] == PRIMARY_INSUFFICIENT_DATA], ["rank"], [True], top_n),
        ),
    ]
...
def _list_payload(list_id: str, *, as_of_date: str | None, top_n: int, items: list[dict[str, object]]) -> dict[str, object]:
